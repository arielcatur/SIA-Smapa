...
export default function ProfileSiswa() {
  ...
  useEffect(() => {
    if (fetchStatus === true) {
      axios
        .get("http://localhost:3000/api/guru/profile", config)
        .then((res) => {
          ...
          setFormData({
            ...
            noTelp: data.noTelp || "",
            alamat: data.alamat || "",
            foto: data.foto || "", // URL foto dari data yang diambil
          });
          setFoto(data.foto);
        })
        .catch((error) => {});
    ...
    try {
      ...
      if (response.data.status) {
        ...
        setFormData((prevData) => ({
          ...prevData,
          foto: response.data.data.foto, // Update formData hanya pada bagian foto
        }));
